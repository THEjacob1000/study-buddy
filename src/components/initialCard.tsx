...
const InitialCard = ({ initial, setInitial }: InitialCardProps) => {
	...
	const onSubmit = async (data: z.infer<typeof questionsSchema>) => {
		...
		try {
			...
			// biome-ignore lint/suspicious/noExplicitAny: This is ok for now
			const formattedData = responseData.map((q: any) => ({
				question: q.question,
				answer: q.answer,
				TimesCorrect: 0,
			}));
			localStorage.setItem("quizQuestions", JSON.stringify(formattedData));
			...
			<Card className="mt-40">
				<CardHeader className="w-full flex items-center justify-center">
					<h2 className="text-xl font-bold px-12 w-3/4 text-center">
						Please Enter Your OpenAI API Key. If you do not have one, one can be
						made{" "}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="w-full flex flex-col items-center">
										...
										{apiError && (
											<p className="text-red-500 mt-4 text-sm">{apiError}</p>
										)}
										<FormControl>
											<Input
												placeholder="Your API key..."
												className={cn("w-1/2", apiError && "border-border")}
												{...field}
												autoFocus={false}
											/>
										</FormControl>
		...
		return (
			<Card className="mt-40">
				<CardHeader className="w-full flex items-center justify-center max-w-[50vw]">
					Please input the questions you&apos;d like to study. You can input
					multiple questions at once, separated by a new line. The AI will
					provide you with a response to each question if you don&apos;t provide
					demo answers
				</CardHeader>
				<CardContent className="w-full flex justify-center items-center text-muted-foreground mt-10">
					<Form {...questionsForm}>
